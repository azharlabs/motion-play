...
(() => {
  ...
  hud.overlay.addEventListener("click", () => { if (state.mode === "ready" || state.mode === "over") start(); else jump(); });
  ...
  function draw(w, h) {
    ...
    g.addColorStop(0, skin === "balance" ? "#042f2e" : "#2e1065"); g.addColorStop(1, "#0f172a");
    ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
    const groundY = h * 0.72;
    for (const p of state.plats) {
      const x = p.x * w, yy = groundY - p.y * h, ww = p.w * w;
      ctx.fillStyle = accent; roundRect(ctx, x, yy, ww, 18, 8); ctx.fill();
    }
    const px = state.x * w, py = groundY - state.y * h - 36;
    ctx.fillStyle = "#f97316"; roundRect(ctx, px - 16, py, 32, 36, 10); ctx.fill();
    ctx.fillStyle = "#fdba74"; ctx.beginPath(); ctx.arc(px, py - 10, 12, 0, Math.PI * 2); ctx.fill();
